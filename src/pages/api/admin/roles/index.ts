...
export const GET: APIRoute = async ({ url, locals }) => {
  ...
  const result = await db.execute({
    sql: 'SELECT id, name, sort_order, manual_content, responsibility_content FROM roles WHERE store_id = ? ORDER BY sort_order',
    args: [storeId],
  });
...
export const POST: APIRoute = async ({ request, locals }) => {
  ...
  if (!name?.trim()) return new Response(JSON.stringify({ error: 'Role name is required' }), { status: 400 });
  const db = getDb();
  const orderResult = await db.execute({
    sql: 'SELECT COALESCE(MAX(sort_order), 0) + 1 AS next FROM roles WHERE store_id = ?',
    args: [storeId],
  });
  const next = (orderResult.rows[0] as unknown as { next: number }).next;
  const result = await db.execute({
    sql: 'INSERT INTO roles (store_id, name, sort_order, manual_content, responsibility_content) VALUES (?, ?, ?, \'\', \'\') RETURNING id, name, sort_order',
